...
export function HomeScreen({
  ...
  return (
    <div style={{ ...S.page, background: bg }}>
      ...
      {streak > 0 && (
        <div style={{ position: 'fixed', top: 20, left: 22, zIndex: 1000, background: '#f59e0b', color: '#fff', padding: '5px 12px', borderRadius: 6, fontSize: 11, fontWeight: 700, letterSpacing: 0.5 }}>
          {streak} DAY STREAK
        </div>
      )}
          ...
          <div className="main-controls">
              ...
              <div style={S.modeRow}>
                {MODES.map(({ id, Icon, label, sub }) => (
                  <button key={id} onClick={() => setMode(id)} onMouseEnter={hov} onMouseLeave={unhov}
                    style={{ ...S.modeBtn, background: mode === id ? pillSel : cardBg, border: `1.5px solid ${mode === id ? pillSel : border}`, color: mode === id ? '#fff' : text, transition: 'opacity 0.15s' }}>
                    <span style={{ color: mode === id ? '#c7d2fe' : subText, display: 'flex' }}><Icon size={20} /></span>
            ...
            {mode !== 'flashcards' && (<>
              ...
              {selChapter !== 'all' && TOPICS_BY_CHAPTER[parseInt(selChapter)]?.length > 0 && (
                <div style={S.section}>
                  <label style={{ ...S.label, color: subText }}>TOPIC DRILL <span style={{ fontSize: 9, fontWeight: 400, letterSpacing: 0 }}>(optional)</span></label>
